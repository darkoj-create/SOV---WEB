-- SOV Arhivar v5.57.1 — BASE STATUS FIX
-- Problem v5.57: worklist je krivo zaključivao da fali nacrt/zapisnik ako ih nema u novoj SOV arhivi.
...
select
  ...
  lower(coalesce(object_name,'') || ' ' || coalesce(plate_number,'') || ' ' || coalesce(nearest_place,'') || ' ' || coalesce(object_type,'') || ' ' || coalesce(field_tasks,'') || ' ' || coalesce(workflow_raw,'')) as search_text,
  updated_at
from enriched;
