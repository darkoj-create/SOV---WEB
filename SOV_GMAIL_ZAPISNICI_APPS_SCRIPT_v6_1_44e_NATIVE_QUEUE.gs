/** SOV Gmail zapisnici v6.1.44e — queue + native text only. No DOCX file upload. */
const SOV_GMAIL_CFG = {
...
function doGet(e) {
  ...
  let result;
  try {
    if (!e || !e.parameter.action) throw new Error('Unknown action');
    ...
    } else if (e.parameter.action === 'queue') {
      result = processSovGmailSyncQueue();
    } else {
...
function processSovGmailSyncQueue() {
  const claim = rpc_('sov_claim_gmail_sync_request', {p_ingest_key:SOV_GMAIL_CFG.INGEST_KEY});
  if (!claim || !claim.request) return {ok:true,status:'idle',message:'No queued Gmail sync request.'};
  const requestId = claim.request.id;
  const days = Math.max(1, Math.min(28, Number(claim.request.requested_days || 28)));
  try {
    const out = processSovGmailZapisnici(days);
    rpc_('sov_finish_gmail_sync_request', {
      p_ingest_key:SOV_GMAIL_CFG.INGEST_KEY,
      p_request_id:requestId,
      p_status: out.failed ? 'completed' : 'completed',
      p_imported_count: out.imported || 0,
      p_skipped_count: out.skipped || 0,
      p_failed_count: out.failed || 0,
      p_result_message: 'Gmail sync dovršen. Native tekst spremljen u meeting_minutes; DOCX nije uploadan.',
      p_meta: out
    });
    return {ok:true,status:'processed',request_id:requestId,result:out};
  } catch (err) {
    rpc_('sov_finish_gmail_sync_request', {
      p_ingest_key:SOV_GMAIL_CFG.INGEST_KEY,
      p_request_id:requestId,
      p_status:'failed',
      p_failed_count:1,
      p_result_message:String(err && err.message || err),
      p_meta:{error:String(err && err.stack || err)}
    });
    throw err;
  }
}
...
function processSovGmailZapisnici(days) {
  ...
  const query = 'newer_than:' + days + 'd has:attachment filename:docx {label:"' + SOV_GMAIL_CFG.LABEL_QUEUE + '" subject:zapisnik subject:Zapisnik}';
  const threads = GmailApp.search(query, 0, 100);
  const out = {ok:true,mode:'native_text_only',days:days,threads:threads.length,messages:0,attachments:0,imported:0,skipped:0,failed:0};
...
function installSovGmailZapisniciTriggers() {
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (['processSovGmailZapisnici','processSovGmailSyncQueue'].indexOf(t.getHandlerFunction()) >= 0) ScriptApp.deleteTrigger(t);
  });
  ScriptApp.newTrigger('processSovGmailSyncQueue').timeBased().everyMinutes(5).create();
...
function ingest_(message, attachment, checksum, text) {
  ...
  return rpc_('sov_ingest_meeting_minutes_from_gmail', payload);
}

function rpc_(fn, payload) {
  const res = UrlFetchApp.fetch(SOV_GMAIL_CFG.SUPABASE_URL + '/rest/v1/rpc/' + fn, {
