-- SOV Oružarstvo v5.43 — canonical sorting, personal-kit shortcut and grouping metadata
-- Run in Supabase SQL editor with RLS enabled.
-- Adds canonical armory grouping metadata for APK/Web: Osobna oprema - komplet, article group keys, display names, details.
...
as $$
  select concat_ws(' ',
    case public.sov_armory_main_category(main_category, main_category)
      ...
      when 'Osobna oprema - komplet' then 'osobna oprema komplet vertikala pojas sjedalica croll krol crol prsni bloker zumar ascender descender spustalica stop rig maestro pedal stremen'
      when 'Osobna oprema' then 'osobna oprema kaciga helmet kombinezon rukavice cizme odjeca obuca'
    ...
    case public.sov_armory_subcategory(subcategory, search_basis)
      ...
      when 'Pedale / stremeni' then 'pedala pedal stremen stremeni'
