/**
 * SOV Izleti Sheet Web App v1.0.87 shared trip coordinates + raspored url
 *
 ...
 * G CenterLat | H CenterLon | I MinLat | J MaxLat | K MinLon | L MaxLon | M RasporedUrl
 */
...
const SOV_TRIPS_HEADERS = [
  ...
  'CenterLat', 'CenterLon', 'MinLat', 'MaxLat', 'MinLon', 'MaxLon', 'RasporedUrl'
];
...
function doGet(e) {
  try {
    ...
    return json_({ ok: true, service: 'SOV Izleti Sheet Web App v1.0.87 shared trip coordinates + raspored url' });
  } catch (err) {
    return json_({ ok: false, error: String(err) });
  }
}
