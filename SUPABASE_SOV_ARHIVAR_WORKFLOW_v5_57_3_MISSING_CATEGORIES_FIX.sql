-- SOV Arhivar v5.57.3 — DATABASE MISSING CATEGORIES FIX
-- Problem v5.57.2: Arhivar vidi puni tekst, ali worklist još ne pokazuje precizno kategorije koje baza kaže da fale.
-- Fix: ne zaključuje iz uploadane SOV arhive, nego parsira postojeću speleo bazu: field_tasks, workflow_raw, record_status, cadastre_status i raw JSON.
-- Dodaje missing_categories/missing_categories_text + pločicu kao posebnu falinku.
...
',
    ...
    (
      b.status_text_norm ~ '((fali|nedostaje|nema|bez|treba|trebaju|potrebno|ponoviti|srediti|dopuniti|prikupiti|pronaci|naci).{0,80}(koordinat|gps|wgs|htrs|lokacij|pozicij|georef))'
    ...
    (
      b.status_text_norm ~ '((fali|nedostaje|nema|bez|treba|trebaju|potrebno|ponoviti|srediti|digitalizirati|napraviti|nacrtati|dopuniti|prikupiti|pronaci|naci).{0,80}(nacrt|tlocrt|profil|poligon|crtez|crtanje|topodroid))'
    ...
    (
      b.status_text_norm ~ '((fali|nedostaje|nema|bez|treba|trebaju|potrebno|srediti|napraviti|dopuniti|prikupiti|pronaci|naci).{0,80}(zapisnik|tzapisnik|izvjestaj|izvjestaj|izvjestaj|izvještaj|bibliograf))'
    ...
    (
      b.status_text_norm ~ '((fali|nedostaje|nema|bez|treba|trebaju|potrebno|postaviti|dodati|upisati|nabaviti).{0,80}(plocic|plocica|plocicu|plocice|broj plocice|oznaka))'
...
select
  ...
  lower(coalesce(object_name,'') || ' ' || coalesce(plate_number,'') || ' ' || coalesce(nearest_place,'') || ' ' || coalesce(object_type,'') || ' ' || coalesce(field_tasks,'') || ' ' || coalesce(workflow_raw,'') || ' ' || coalesce(base_details_text,'') || ' ' || coalesce(source_missing_text,'')) as search_text,
  updated_at
from enriched;
...
-- Verification: brojke poslije fixa pokazuju eksplicitne falinke iz baze: pločica, nacrt, zapisnik, koordinate.
