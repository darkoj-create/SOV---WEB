-- SOV Arhivar v5.58.0 — Predane jame / submission → review → approve → baza
...
using (
  ...
  or exists (
    select 1 from public.sov_user_profiles p
    where p.user_id = auth.uid()
      and coalesce(p.status::text,'') = 'approved'
      and coalesce(p.role::text,'') in ('admin','arhivar')
  )
);
...
begin
  ...
  insert into public.speleo_objects_staging (
    ...
    'sov_arhivar_submission_v5_58_0'
  )
