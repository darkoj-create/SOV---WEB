            ...
            Card(
                ...
                Column(
                    ...
                    Box(
                        modifier = Modifier
                            .size(92.dp)
                            .background(Color(0xFFFFD58A).copy(alpha = 0.16f), RoundedCornerShape(30.dp)),
                        contentAlignment = Alignment.Center
                    ) {
                        Image(
                            painter = painterResource(id = R.drawable.logo_sov),
                            contentDescription = "SOV",
                            modifier = Modifier.size(62.dp)
                        )
                    }
                    Text(
                        text = if (session.isLoggedIn) language.pick("Prijava je spremljena", "Sign-in is saved") else language.pick("Prijavi se u SOV Cloud", "Sign in to SOV Cloud"),
                    ...
                    Text(
                        text = language.pick(
                            "App ostaje prijavljen i nakon updatea. Role i permissioni se osvježavaju automatski kad ima mreže, a offline koristi spremljeni cache.",
                            "The app stays signed in after updates. Roles and permissions refresh automatically online, and offline mode uses the saved cache."
                        ),
                    ...
                    if (session.isLoggedIn) {
                        ...
                        Button(
                            ...
                            Text(language.pick("Nastavi u aplikaciju", "Continue to app"))
                        }
                        OutlinedButton(
                            onClick = { signOut() },
                            enabled = !busy,
                        ...
                        Button(
                            ...
                            if (busy) CircularProgressIndicator(modifier = Modifier.size(18.dp), strokeWidth = 2.dp) else Icon(Icons.Default.Security, contentDescription = null)
                            Spacer(Modifier.width(8.dp))
                            Text(language.pick("Prijavi me", "Sign me in"), fontWeight = FontWeight.Bold)
                        }
                    }
