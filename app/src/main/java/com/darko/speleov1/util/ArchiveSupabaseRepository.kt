...
internal object ArchiveCloudCache {
    private const val PREFS = "sov_archive_work_cache_v6_full_worklist"
...
internal object ArchiveSupabaseRepository {
    ...
    fun loadObjectDetail(context: Context, base: ArchiveWorkItem): ArchiveWorkItem {
        ...
        val txt = SovHttpClient.post(
            context = context,
            url = "$SOV_SUPABASE_URL/rest/v1/rpc/sov_arhivar_get_object_detail_v2",
            body = body
        )
    ...
    private fun fetchWorklist(context: Context): List<ArchiveWorkItem> {
        // Keep the APK in sync with web Arhivar/Karta truth: PostgREST/Supabase
        // can silently cap a single request, so we page through the full view.
        // This fixes the old APK-only 800 object limit while web shows 1285+.
        val pageSize = 1000
        val maxPages = 25
        val all = linkedMapOf<String, ArchiveWorkItem>()
        var offset = 0
        repeat(maxPages) {
            val txt = SovHttpClient.get(
                context = context,
                url = "$SOV_SUPABASE_URL/rest/v1/sov_arhivar_worklist?select=*&order=priority_score.desc,object_name.asc&limit=$pageSize&offset=$offset"
            )
            val page = JSONArray(txt).mapObjects { it.toArchiveWorkItem() }
            page.forEach { item ->
                val key = item.objectId.ifBlank { "${item.objectName}|${item.plateNumber}|${item.nearestPlace}" }
                all[key] = item
            }
            if (page.size < pageSize) return all.values.toList()
            offset += pageSize
        }
        return all.values.toList()
    }

    private fun fetchStats(context: Context): ArchiveDashboardStats? = runCatching {
        val arr = JSONArray(SovHttpClient.get(context, "$SOV_SUPABASE_URL/rest/v1/sov_arhivar_dashboard?select=*&limit=1"))
