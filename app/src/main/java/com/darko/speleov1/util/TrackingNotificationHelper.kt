...
object TrackingNotificationHelper {
    ...
    private const val CHANNEL_NAME = "SOV Field Tracking"
    ...
    fun buildNotification(context: Context, waitingForGpsFix: Boolean, startedAtMillis: Long? = null): Notification {
        ...
        val ft = FieldTrackingLitePrefs.load(context)
        val queue = FieldTrackingLiteStore.pendingCount(context)
        val text = if (ft.active) {
            if (waitingForGpsFix) "SOV tracking uključen • tražim GPS" else "SOV tracking uključen • queue: ${queue}"
        } else if (waitingForGpsFix) "Live tracking radi • tražim GPS fix" else "Live tracking radi"
        return NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(R.mipmap.ic_launcher_round)
            .setContentTitle("SOV Field Tracking")
            .setContentText(text)
            .setOngoing(true)
    ...
    private fun ensureChannel(context: Context) {
        ...
        val channel = NotificationChannel(CHANNEL_ID, CHANNEL_NAME, NotificationManager.IMPORTANCE_LOW).apply {
            description = "Prikazuje da je SOV Field Tracking aktivan"
            setShowBadge(false)
        }
