-- SOV Arhivar v5.57.8 — ARHIVAR workflow checkboxes + TK25 link support
...
-- v5.57.8 dodatno: status tablica pamti pločicu/fotku/ponavljanje nacrta i worklist ih vraća kao normalne checkove.
...
-- Minimal safety: status table exists from v5.57, but create it if this patch is run on a fresh DB.
...
-- v5.57.8: prošireni arhivarski checklist. Safe alter; stari web/APK i dalje rade.
alter table public.sov_archive_object_status add column if not exists has_plate boolean;
alter table public.sov_archive_object_status add column if not exists has_photo boolean;
alter table public.sov_archive_object_status add column if not exists needs_redraw boolean;
alter table public.sov_archive_object_status add column if not exists workflow_checks jsonb not null default '{}'::jsonb;
...
with base as (
  ...
  select
    ...
    (
      public.sov_txt_norm(coalesce(b.field_tasks,'')) ~ '(^|[^a-z0-9])(fotka|foto|slika|fotograf)([^a-z0-9]|$)'
      or public.sov_txt_norm(coalesce(b.workflow_raw,'')) ~ '(^|[^a-z0-9])(fotka|foto|slika|fotograf)([^a-z0-9]|$)'
      or b.status_text_norm ~ '((fali|nedostaje|nema|bez|treba|potrebno|napraviti|dodati).{0,80}(fotka|foto|slika|fotograf))'
    ...
    (
      public.sov_txt_norm(coalesce(b.field_tasks,'')) ~ '(ponoviti_nacrt|nacrt_ponoviti|ponoviti.{0,40}nacrt|ponovno.{0,40}nacrt)'
      or public.sov_txt_norm(coalesce(b.workflow_raw,'')) ~ '(ponoviti_nacrt|nacrt_ponoviti|ponoviti.{0,40}nacrt|ponovno.{0,40}nacrt)'
      or b.status_text_norm ~ '((ponoviti|ponovno|los|loš|necitljiv|nečitljiv|stari).{0,80}(nacrt|tlocrt|profil|crtez|crtež))'
  ...
  select
    ...
    st.priority,
    st.last_note,
    coalesce(nullif(st.plate_number,''), r.plate_number) as effective_plate_number,
    coalesce(st.has_plate,
      case
        when coalesce(nullif(st.plate_number,''), r.plate_number, '') <> '' then true
        when r.base_in_cadastre or r.base_ready_for_katastar then true
        when r.explicit_missing_plate then false
        else true
      end
    ) as has_plate,
    coalesce(st.has_photo, case when r.explicit_missing_photo then false else true end) as has_photo,
    coalesce(st.needs_redraw, r.explicit_needs_redraw) as needs_redraw,
...
select
  object_id,
  object_name,
  effective_plate_number as plate_number,
  object_type,
  nearest_place,
  lat,
  lon,
  ...
  explicit_missing_photo,
  explicit_needs_redraw,
  source_missing_text,
  (not has_coordinates) as missing_coordinates,
  (not has_drawing) as missing_drawing,
  (not has_record) as missing_record,
  (not has_plate) as missing_plate,
  array_remove(array[
    case when not has_plate then 'pločica' end,
    ...
    case when not has_photo then 'fotka' end,
    case when needs_redraw then 'ponoviti nacrt' end
  ...
  case
    ...
    when (not has_plate) or (not has_coordinates) or (not has_drawing) or (not has_record) or (not has_photo) or needs_redraw then 'nepotpuno'
  ...
  (case when not has_coordinates then 30 else 0 end
   ...
   + case when not has_plate then 15 else 0 end
   + case when not has_photo then 8 else 0 end
  ...
  lower(coalesce(object_name,'') || ' ' || coalesce(effective_plate_number,'') || ' ' || coalesce(nearest_place,'') || ' ' || coalesce(object_type,'') || ' ' || coalesce(field_tasks,'') || ' ' || coalesce(workflow_raw,'') || ' ' || coalesce(source_missing_text,'')) as search_text,
  updated_at
from enriched;
...
select
  ...
  count(*) filter (where not has_photo)::integer as missing_photos,
  count(*) filter (where needs_redraw)::integer as needs_redraw_count,
...
begin
  with base as (
    select
      s.source_id as object_id,
      coalesce(nullif(o.data->>'name',''), s.name) as object_name,
      coalesce(nullif(st.plate_number,''), nullif(o.data->>'plate_number',''), nullif(o.data->>'plocica',''), nullif(o.data->>'pločica',''), nullif(o.data->>'cadastral_number',''), nullif(s.plate_number,''), nullif(s.cadastral_number,''), public.sov_json_first_text((to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)),'plate_number','plocica','pločica','broj_plocice','broj_pločice','broj plocice','broj pločice','broj_plocice_na_objektu','broj_pločice_na_objektu','cadastral_number','katastarski_broj','katastarski broj','kat_broj','kbr','oznaka','plocica_broj','pločica_broj')) as plate_number,
      coalesce(nullif(o.data->>'object_type',''), s.object_type_final) as object_type,
      coalesce(nullif(o.data->>'nearest_place',''), s.nearest_place, s.locality, s.municipality) as nearest_place,
      coalesce(case when (o.data->>'lat') ~ '^-?[0-9]+(\.[0-9]+)?$' then (o.data->>'lat')::double precision else null end, s.lat) as lat,
      coalesce(case when (o.data->>'lon') ~ '^-?[0-9]+(\.[0-9]+)?$' then (o.data->>'lon')::double precision else null end, s.lon) as lon,
      ...
      st.needs_redraw,
      coalesce(st.workflow_checks, '{}'::jsonb) as workflow_checks,
      to_jsonb(s) as sql_row_json,
      (to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)) as merged_sql_json,
      concat_ws(E'\n',
        ...
        nullif('Pločica: ' || coalesce(coalesce(nullif(st.plate_number,''), nullif(o.data->>'plate_number',''), nullif(o.data->>'plocica',''), nullif(o.data->>'pločica',''), nullif(o.data->>'cadastral_number',''), nullif(s.plate_number,''), nullif(s.cadastral_number,''), public.sov_json_first_text((to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)),'plate_number','plocica','pločica','broj_plocice','broj_pločice','broj plocice','broj pločice','broj_plocice_na_objektu','broj_pločice_na_objektu','cadastral_number','katastarski_broj','katastarski broj','kat_broj','kbr','oznaka','plocica_broj','pločica_broj')),''), 'Pločica: '),
        ...
        nullif('Opis: ' || coalesce(public.sov_json_first_text((to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)),'technical_description','opis_objekta','opis objekta','tehnicki_opis','tehnički_opis','description','opis','Opis','description_hr','morphology','morfologija'), ''), 'Opis: '),
        nullif('Pristup: ' || coalesce(public.sov_json_first_text((to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)),'access_description','pristup','opis_pristupa','access','approach','lokacija','location_description'), ''), 'Pristup: '),
        nullif('Istraživanje / povijest: ' || coalesce(public.sov_json_first_text((to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)),'research','istrazivanje','istraživanje','exploration','history','povijest','opis_istrazivanja','opis istraživanja'), ''), 'Istraživanje / povijest: '),
        nullif('Autori / ekipa: ' || coalesce(public.sov_json_first_text((to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)),'authors','autori','author','autor','members','clanovi','članovi','ekipa','tko','team'), ''), 'Autori / ekipa: '),
        nullif('Hidrologija: ' || coalesce(public.sov_json_first_text((to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)),'hydrology','hidrologija','hydrogeology','hidrogeologija'), ''), 'Hidrologija: '),
        nullif('Geologija / morfologija: ' || coalesce(public.sov_json_first_text((to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)),'geology','geologija','morphology','morfologija'), ''), 'Geologija / morfologija: '),
        nullif('Opasnosti / zaštita: ' || coalesce(public.sov_json_first_text((to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)),'hazards','opasnosti','observed_threats','ugroze','protection','zastita','zaštita'), ''), 'Opasnosti / zaštita: '),
        nullif('Napomena: ' || coalesce(public.sov_json_first_text((to_jsonb(s) || coalesce(s.raw,'{}'::jsonb)),'note','napomena','Napomena','remarks','komentar'), ''), 'Napomena: '),
        nullif('--- SVA SQL POLJA IZ speleo_objects_staging ---' || E'\n' || coalesce(jsonb_pretty(to_jsonb(s)),''), '--- SVA SQL POLJA IZ speleo_objects_staging ---' || E'\n'),
        nullif('--- RAW IMPORT JSON ---' || E'\n' || coalesce(jsonb_pretty(coalesce(s.raw,'{}'::jsonb)),''), '--- RAW IMPORT JSON ---' || E'\n{}')
  ...
  select jsonb_build_object(
    ...
    'has_plate', b.has_plate,
    'has_photo', b.has_photo,
    'needs_redraw', b.needs_redraw,
    'workflow_checks', b.workflow_checks,
    ...
    'raw_text', b.raw_text,
    'sql_row_text', jsonb_pretty(coalesce(b.sql_row_json,'{}'::jsonb)),
    'all_sql_fields_text', jsonb_pretty(coalesce(b.merged_sql_json,'{}'::jsonb)),
...
-- v5.57.8: extended RPC for Arhivar UI. Old sov_archive_update_object_status remains for APK/backward compatibility.
create or replace function public.sov_archive_update_object_status_v2(
  p_object_id text,
  p_object_name text,
  p_plate_number text,
  ...
  p_has_plate boolean default null,
  p_has_photo boolean default null,
  p_needs_redraw boolean default null,
  p_extra_checks jsonb default '{}'::jsonb
...
declare
  ...
  v_has_plate boolean;
  v_has_photo boolean;
  v_needs_redraw boolean;
begin
  if not public.sov_is_arhivar_or_admin() then
    raise exception 'Samo Admin/Arhivar može mijenjati status arhive.';
  end if;

  v_has_plate := coalesce(p_has_plate, nullif(trim(coalesce(p_plate_number,'')), '') is not null);
  v_has_photo := coalesce(p_has_photo, true);
  v_needs_redraw := coalesce(p_needs_redraw, false);

  insert into public.sov_archive_object_status(
    object_id, object_name, plate_number,
    has_coordinates, has_drawing, has_record, has_plate, has_photo, needs_redraw,
    has_katastar_ready, archive_status, priority, last_note, workflow_checks, metadata, updated_by, updated_at
  ) values (
    p_object_id, p_object_name, p_plate_number,
    p_has_coordinates, p_has_drawing, p_has_record, v_has_plate, v_has_photo, v_needs_redraw,
    coalesce(v_has_plate,false) and coalesce(p_has_coordinates,false) and coalesce(p_has_drawing,false) and coalesce(p_has_record,false) and coalesce(v_has_photo,false) and not coalesce(v_needs_redraw,false),
    coalesce(nullif(p_archive_status,''),'needs_review'),
    coalesce(nullif(p_priority,''),'normal'),
    p_note,
    coalesce(p_extra_checks, '{}'::jsonb),
    jsonb_build_object('module','arhivar','ui','web','version','5.57.8') || coalesce(p_extra_checks, '{}'::jsonb),
    auth.uid(), now()
  )
  on conflict (object_id) do update set
    ...
    needs_redraw = excluded.needs_redraw,
    has_katastar_ready = excluded.has_katastar_ready,
    archive_status = excluded.archive_status,
    priority = excluded.priority,
    last_note = excluded.last_note,
    workflow_checks = excluded.workflow_checks,
    metadata = coalesce(public.sov_archive_object_status.metadata,'{}'::jsonb) || excluded.metadata,
    updated_by = auth.uid(),
    updated_at = now()
...
grant execute on function public.sov_archive_update_object_status_v2(text,text,text,boolean,boolean,boolean,text,text,text,boolean,boolean,boolean,jsonb) to authenticated;
