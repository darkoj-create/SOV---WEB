...
-- -------------------------------------------------------------
-- SOV web v6.0.3 — Predaj novu jamu standalone form + XLSX export safety patch
-- Idempotentno. Ako su v5.58.1 submissions tablice već pokrenute, ovo samo doda
-- kompatibilne stupce i health RPC za sync-status / dijagnostiku.
-- -------------------------------------------------------------
...
alter table if exists public.speleo_object_submissions
  ...
  add column if not exists entrance_altitude_m double precision,
  add column if not exists surveyors text,
  add column if not exists drawn_by text,
  add column if not exists plate_code text,
  add column if not exists cadastre_hint text;

create or replace function public.sov_speleo_submissions_health()
returns jsonb
language plpgsql
security definer
set search_path = public
...
declare
  ...
  v_approved int := 0;
  v_files int := 0;
  v_has_bucket boolean := false;
begin
  if to_regclass('public.speleo_object_submissions') is not null then
    execute 'select count(*) from public.speleo_object_submissions' into v_submissions;
    execute 'select count(*) from public.speleo_object_submissions where status in (''submitted'',''needs_changes'',''draft'')' into v_pending;
    execute 'select count(*) from public.speleo_object_submissions where status = ''approved''' into v_approved;
  end if;
  if to_regclass('public.speleo_object_submission_files') is not null then
    execute 'select count(*) from public.speleo_object_submission_files' into v_files;
  end if;
  if exists (select 1 from information_schema.tables where table_schema='storage' and table_name='buckets') then
    select exists(select 1 from storage.buckets where id='speleo-submissions') into v_has_bucket;
  end if;
  return jsonb_build_object(
    ...
    'files', v_files,
    'bucket', v_has_bucket,
    'form_page', 'predaj-novu-jamu.html',
    'review_page', 'arhivar-predane-jame.html',
    'export', jsonb_build_array('xlsx','csv','xml','zip')
  );
end;
$$;

grant execute on function public.sov_speleo_submissions_health() to authenticated;

comment on function public.sov_speleo_submissions_health() is 'SOV v6.0.3 health check for Predaj novu jamu / Arhivarski inbox / XLSX export.';
