-- SOV Cloud v5.58.19 — karta.html uses the same source as Arhivar
-- Fixes: old set-returning map RPC / PostgREST cap of 1000 rows, wrong status source and negative "nije u katastru" truth,
...
'SOV Cloud v5.58.19 paged map feed. Primary source is sov_arhivar_worklist, matching Arhivar missing-category/status logic. Returns a single JSON object to avoid PostgREST 1000-row truncation.';


-- v5.58.19 note: frontend status logic now checks negative Arhivar readiness/missing categories before any green 'u katastru' phrase.
