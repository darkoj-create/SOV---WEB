...
(function(){
  ...
  const PUBLIC_PAGES=new Set([
    'index.html','vijesti.html','vijest.html','o-drustvu.html','povijest.html','procelnistvo.html','velebitaski-duh.html','speleoskola.html','pridruzi-nam-se.html','videos.html','login.html'
  ]);
  ...
  const PUBLIC_LINKS=[
    ...
    {href:'o-drustvu.html',label:'O nama',group:'SOV'},
    {href:'speleoskola.html',label:'Speleoškola',group:'SOV'},
    {href:'pridruzi-nam-se.html',label:'Pridruži nam se',group:'SOV'},
    {href:'dashboard.html',label:'SOV Cloud',group:'SOV'}
  ];
  const LINKS=[
  ...
  function ensureHeader(role){
    if(!SHELL_PAGES.has(PAGE)) return;
    ...
    if(brand && !IS_PUBLIC && !brand.querySelector('b')){
  ...
  function makeDrawer(role){
    ...
    if(drawer && drawer.dataset.sovDrawerRole===String(role||'')) return drawer;
    if(drawer) drawer.remove();
    drawer=document.createElement('div'); drawer.className='sov-shell-drawer'; drawer.dataset.sovDrawerRole=String(role||''); drawer.setAttribute('aria-hidden','true');
    const groups=IS_PUBLIC ? [['SOV', PUBLIC_LINKS]] : groupLinks(visibleLinks(role));
  ...
  function hideUnauthorized(role){
    ...
    if(!['webmaster','admin','oruzar'].includes(role)) document.querySelectorAll('a[href="oruzarstvo-import.html"],a[href^="oruzar-master"]').forEach(el=>el.classList.add('sov-shell-hidden-for-role'));
  ...
  function bindGlobalMenu(){
    ...
    window.__SOV_SHELL_GLOBAL_MENU_BOUND__=true;
    document.addEventListener('click',function(e){
      const btn=e.target && e.target.closest ? e.target.closest('[data-sov-shell-menu]') : null;
      if(!btn) return;
      e.preventDefault();
      e.stopPropagation();
      const role=(document.body && document.body.dataset && document.body.dataset.sovRole) || (IS_PUBLIC ? 'public' : 'user');
      openDrawer(role);
    },true);
  }
