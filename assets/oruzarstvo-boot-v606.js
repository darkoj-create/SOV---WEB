/* SOV Oružarstvo single boot v6.0.6
...
(function(){
  'use strict';
  const BUILD='6.0.6';
  const STATIC_URL='data/oruzarstvo-data.json?b=606';
  const CACHE_KEY='sov_armory_catalog_cache_v606';
  ...
  function available(r){return (Number(r.available)>0)||/dostup|aktiv|društvu|drustvu/i.test(String(r.availability||r.status||''));}
  ...
  function rows(){
    ...
    const ropes=(Array.isArray(d.ropes)?d.ropes:[]).map(r=>({
      ...
      category:r.category||r.category_name||'Užad i užetna oprema',
      category_name:r.category_name||r.category||'Užad i užetna oprema',
  ...
  function categoryMeta(c){const x=plain(c); if(x.includes('uze'))return ['🪢','Užad i užetna oprema']; if(x.includes('osob'))return ['🧗','Osobna speleo oprema']; if(x.includes('postavlj'))return ['🪛','Postavljanje i sidrišta']; if(x.includes('busil'))return ['🔋','Bušilice, svrdla i aku sustav']; if(x.includes('elektro')||x.includes('foto')||x.includes('rasv'))return ['💡','Rasvjeta, baterije i foto']; if(x.includes('crtan')||x.includes('mjeren')||x.includes('kompas')||x.includes('busol'))return ['🧭','Crtanje, mjerenje i busole']; if(x.includes('dron'))return ['🚁','Dronovi i dodaci']; if(x.includes('alat'))return ['🧰','Alat i radionica']; return ['📦','Oprema'];}
  function sortCats(a){const order=['Osobna oprema','Užad i užetna oprema','Oprema za postavljanje','Bušilice i svrdla','Oprema za crtanje','Elektro, rasvjeta i foto','Dronovi','Alat i radionica']; return a.sort((x,y)=>(order.indexOf(x)<0?99:order.indexOf(x))-(order.indexOf(y)<0?99:order.indexOf(y))||String(x).localeCompare(String(y),'hr'));}
  function currentItems(){const q=plain($('q')?.value||''); const cat=$('cat')?.value||activeCat; const av=$('avail')?.value||''; return rows().filter(visible).filter(r=>!q||rowText(r).includes(q)).filter(r=>!cat||catOf(r)===cat).filter(r=>!activeSub||subOf(r)===activeSub).filter(r=>!av||String(r.availability||r.status||'').toLowerCase().includes(av)|| (av==='dostupno'&&available(r)) || (av==='nedostupno'&&!available(r)));}
  function statusLine(total,kind='ok'){return `<div class="v606-status-line"><span class="v606-pill ${kind}">${esc(activeSource)} · ${total} stavki</span><span class="v606-pill">single boot ${BUILD}</span><span class="v606-pill">static first · live background</span></div>`;}
  function setMiniStatus(kind,msg){const el=$('armoryBootStatus')||document.querySelector('[data-armory-boot-status]'); if(el)el.textContent=msg; const line=$('v606StatusText'); if(line)line.textContent=msg;}
  ...
  function renderCatalog(){
    ...
    if(cat&&!activeSub&&!q){
      ...
      el.innerHTML=status+`<div class="user-path"><button onclick="sovArmory606.clear()">Sve kategorije</button><button class="active">${esc(cat)}</button></div><div class="v606-sub-head"><div><h2>${esc(cat)}</h2><p class="fineprint">Odaberi podkategoriju ili kreni pretraživati unutar kategorije.</p></div><button class="btn" onclick="sovArmory606.clear()">← Kategorije</button></div><div class="user-subcat-list">${subs.map(s=>`<button class="user-subcat-card" onclick="sovArmory606.pickSub('${esc(s)}')">${esc(s)}<small>${base.filter(r=>subOf(r)===s).length} stavki</small></button>`).join('')}</div>`;
      return;
    }
    const items=currentItems(); const title=activeSub||cat||(q?'Rezultati':'Oprema');
    el.innerHTML=status+`<div class="user-path"><button onclick="sovArmory606.clear()">Sve kategorije</button>${cat?`<button onclick="sovArmory606.backToCat()">${esc(cat)}</button>`:''}${activeSub?`<button class="active">${esc(activeSub)}</button>`:''}</div><div class="v606-sub-head"><div><h2>${esc(title)}</h2><p class="fineprint">Klikni Zatraži. Količinu i napomenu urediš u zahtjevu.</p></div><button class="btn" onclick="sovArmory606.back()">← Nazad</button></div>${items.length?`<div class="grid-list">${items.map(r=>{const c=catOf(r),s=subOf(r),m=categoryMeta(c),ok=available(r);return `<article class="item-card"><div class="meta"><span class="badge">${m[0]} ${esc(c)}</span><span class="badge">${esc(s)}</span></div><h3>${esc(nameOf(r))}</h3>${r.internal_note||r.note?`<p class="fineprint">${esc(String(r.internal_note||r.note)).slice(0,180)}</p>`:''}<div class="actions"><button class="btn primary" ${ok?'':'disabled'} onclick="sovArmory606.add('${esc(idOf(r))}')">${ok?'Zatraži':'Nedostupno'}</button></div></article>`}).join('')}</div>`:'<div class="v606-empty">Nema artikala za ovaj izbor.</div>'}`;
  }
  ...
  async function loadStatic(){try{return await fetchJson(STATIC_URL,STATIC_TIMEOUT);}catch(e){console.warn('SOV armory v606 static failed',e?.message||e); return null;}}
  async function loadLive(){ if(liveRefreshing)return; liveRefreshing=true; try{ if(!(window.SOVArmoryDB&&SOVArmoryDB.configured&&SOVArmoryDB.configured()&&SOVArmoryDB.loadAllData))return; const live=await Promise.race([SOVArmoryDB.loadAllData({force:true}),timeout(LIVE_TIMEOUT,{__timeout:true})]); if(live&&live.__timeout){setMiniStatus('warn',`Statički katalog radi · Supabase timeout nakon ${LIVE_TIMEOUT/1000}s`);return;} if(hasRows(live)) applyData(live,'Supabase live'); }catch(e){console.warn('SOV armory v606 live skipped',e?.message||e); setMiniStatus('warn','Statički katalog radi · Supabase nije osvježen');} finally{liveRefreshing=false;} }
  async function loadRequests(){try{ if(typeof loadRequestsFromBackend==='function') await Promise.race([loadRequestsFromBackend(),timeout(2500,null)]); if(typeof renderMyRequests==='function')renderMyRequests();}catch(e){console.warn('SOV armory v606 requests skipped',e?.message||e);} }
  ...
  async function boot(){
    const el=$('catalog'); if(el)el.innerHTML='<div class="v606-load-note"><b>Učitavam lokalni katalog opreme…</b><small>Ovaj boot ne čeka Supabase, request tablice ni stare fallback skripte.</small></div>';
    clearOldCache(); attach();
    const cached=readCache(); if(cached&&hasRows(cached.data))applyData(cached.data,'Cache v606');
