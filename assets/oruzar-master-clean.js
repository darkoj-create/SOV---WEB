(function(){
  ...
  function renderInventory(){
    ...
    let title='Inventar opreme', hint='Kreni od kategorije, zatim podkategorije. Pretraga radi po nazivu, modelu, lokaciji i napomeni.';
    let html='';
    if(!cat){
  ...
  function itemCard(r){
    ...
    const note=(r.raw&&(r.raw.physical_code_note||r.raw.note||r.raw.internal_note||''))||'';
    return `<article class="item-card item-card-clean ${low?'low-stock':''} ${r.needsCount?'needs-count':''}"><div class="item-head"><div><h3>${esc(r.name)}</h3><div class="muted">${esc(r.category)} · ${esc(r.subcategory)}</div></div><span class="badge ${statusClass}">${esc(statusText)}</span></div><div class="badgetray"><span class="badge">${esc(r.location||'bez lokacije')}</span><span class="badge">${esc(r.unit)}</span>${r.lastInventoryDate?`<span class="badge">inventura ${esc(r.lastInventoryDate)}</span>`:''}${r.sourceXlsRow?`<span class="badge">XLS red ${esc(r.sourceXlsRow)}</span>`:''}${r.variants&&r.variants>1?`<span class="badge">${esc(r.variants)} varijanti</span>`:''}${r.minimum?`<span class="badge warn">prag ${esc(r.minimum)}</span>`:''}</div><div class="stock stock-clean"><span><b>${esc(qtyDisplay)}</b><em>ukupno</em></span><span><b>${esc(avDisplay)}</b><em>dostupno</em></span><span><b>${esc(r.loan)}</b><em>vani</em></span></div>${note?`<div class="muted item-note">${esc(note).slice(0,220)}</div>`:''}<div class="cm-tools item-actions"><button class="cm-btn" onclick="CleanArmory.editItem('${esc(r.id)}')">Uredi</button><button class="cm-btn bad" onclick="CleanArmory.removeItem('${esc(r.id)}')">Makni</button></div></article>`;
  }
