(function(){
  ...
  function renderMaster(){
    ...
    root.innerHTML=`<div class="cm-grid"><a class="cm-card" href="oruzar-master-posudbe.html"><h2>Posudba</h2><p><b>${requested}</b> za izdati · <b>${issued}</b> vani. Zahtjevi, izdavanje i povrat opreme.</p><span>Otvori →</span></a><button class="cm-card cm-card-button" onclick="CleanArmory.openLegacyReturn()"><h2>Povrat stare opreme</h2><p>Za opremu koja se vratila iz stare / loše evidentirane posudbe. Ne traži otvorenu posudbu.</p><span>Zaprimanje →</span></button><a class="cm-card" href="oruzar-master-inventar.html"><h2>Inventar</h2><p>Pregled artikala po kategorijama, s brzim uređivanjem i preuzimanjem popisa.</p><span>Otvori →</span></a><a class="cm-card" href="oruzar-master-inventura.html"><h2>Inventura</h2><p>Brzo brojanje i spremanje izvještaja po kategorijama.</p><span>Otvori →</span></a><a class="cm-card" href="oruzar-master-notes.html"><h2>Bilješke</h2><p><b>${low}</b> stavki je ispod praga ili treba pažnju.</p><span>Otvori →</span></a></div>`;
  }
  ...
  async function issueLoan(id){
    ...
    catch(e){console.warn('[armory master v5.47.3] issue failed',e); await setStatus(id,'issued',{skipConfirm:true}); return;}
    ...
    await loadRequests(); await renderLoans(); renderMaster();
    // Poništi za izdavanje nije automatski dodan jer izdavanje može stvoriti povezanu posudbu u bazi.
    // Sigurno poništavanje treba poseban backend/API koji vraća i zahtjev i povezane stavke posudbe u prethodno stanje.
    toast('Oprema izdana.');
  }
