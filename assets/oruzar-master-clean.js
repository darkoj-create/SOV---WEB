(function(){
  ...
  function categoryName(row,type){
    // v5.46: SQL view is the only canonical brain. Client only reads fields/falls back.
  ...
  async function loadData(){
    ...
    try{ if(window.SOVArmoryDB&&SOVArmoryDB.configured&&SOVArmoryDB.configured()&&SOVArmoryDB.loadAllData){ d=await SOVArmoryDB.loadAllData(); if(d) source='supabase'; }}catch(e){console.warn('[armory master v5.46] Supabase catalog failed',e)}
    if(!d){ try{ d=await fetch('data/oruzarstvo-data.json',{cache:'no-store'}).then(r=>r.json()); }catch(e){console.warn('[armory master v5.46] static catalog failed',e); d={items:[],ropes:[],pieces:[],categories:[]}; }}
  ...
  async function loadRequests(){
    let req=null; try{ if(window.SOVArmoryDB&&SOVArmoryDB.configured&&SOVArmoryDB.configured()&&SOVArmoryDB.loadRequests) req=await SOVArmoryDB.loadRequests(); }catch(e){console.warn('[armory master v5.46] load requests failed',e)}
  ...
  async function setStatus(id,status){
    try{ if(window.SOVArmoryDB&&SOVArmoryDB.configured&&SOVArmoryDB.configured()&&SOVArmoryDB.updateRequestStatus) await SOVArmoryDB.updateRequestStatus(id,status); }catch(e){console.warn('[armory master v5.46] remote status failed',e); toast('Supabase update nije prošao; spremam lokalno.');}
  ...
  async function issueLoan(id){
    ...
    catch(e){console.warn('[armory master v5.46] issue failed',e); await setStatus(id,'issued'); return;}
  ...
  async function confirmReturn(id){
    ...
    catch(e){console.warn('[armory master v5.46] return failed',e); toast('Supabase povrat nije prošao; spremam lokalno.');}
  ...
  async function saveItem(ev,id,isNew){
    ...
    catch(e){ console.warn('[armory master v5.46] save item local fallback',e); const local=JSON.parse(localStorage.getItem('sov_armory_items_override')||'[]'); const i=local.findIndex(x=>String(x.legacy_id)===String(payload.legacy_id)); if(i>=0)local[i]=payload; else local.unshift(payload); localStorage.setItem('sov_armory_items_override',JSON.stringify(local)); }
