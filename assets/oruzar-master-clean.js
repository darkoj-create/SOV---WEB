...
function renderMaster(){const root=document.getElementById('masterRoot'); if(!root)return; renderKpis(); root.innerHTML=`<div class="cm-grid"><a class="cm-card" href="oruzar-master-inventar.html"><span class="ico">📦</span><h2>Inventar</h2><p>Kategorije, podkategorije, artikli, dostupno/ukupno/posuđeno i lokacije.</p><span>Otvori →</span></a><a class="cm-card" href="oruzar-master-inventura.html"><span class="ico">✅</span><h2>Inventura</h2><p>Nova inventura, datum, kategorije i potvrda stvarnog broja artikala.</p><span>Otvori →</span></a><a class="cm-card" href="oruzar-master-posudbe.html"><span class="ico">↔️</span><h2>Posudba</h2><p>Zatražene posudbe i izdano vani. Vraćeno nestaje iz aktivnog pregleda.</p><span>Otvori →</span></a></div>`}
...
function newItem(){toast('Dodavanje artikla ostaje kroz Inventar formu; neće rušiti prikaz kategorija.');}
function editItem(id){toast('Edit artikla: '+id)}
function removeItem(id){toast('Micanje/rashod artikla: '+id)}
async function init(){await loadData(); renderKpis(); renderMaster(); renderInventory(); await renderLoans();}
window.CleanArmory={init,pickCat(c){STATE.cat=c||null;STATE.sub=null;renderInventory()},pickSub(s){STATE.sub=s||null;renderInventory()},renderLoans,setStatus,manualLoan,newItem,editItem,removeItem};
