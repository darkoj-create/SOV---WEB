(function(){
  ...
  function renderInventory(){
    ...
    const activeRows=allRows.filter(r=>!/(rashod|otpis|deleted|obrisano)/i.test(String(r.status||'')));
  ...
  function itemCard(r){
    ...
    const statusClass=low?'bad':(r.av>0?'ok':'bad');
    return `<article class="item-card item-card-clean ${low?'low-stock':''}"><div class="item-head"><div><h3>${esc(r.name)}</h3><div class="muted">${esc(r.category)} · ${esc(r.subcategory)}</div></div><span class="badge ${statusClass}">${low?'ispod praga':(r.av>0?'dostupno':'nema dostupno')}</span></div><div class="badgetray"><span class="badge">${esc(r.location||'bez lokacije')}</span>${r.variants&&r.variants>1?`<span class="badge">${esc(r.variants)} varijanti</span>`:''}${r.minimum?`<span class="badge warn">prag ${esc(r.minimum)}</span>`:''}${r.type==='rope'?'<span class="badge warn">kodirano uže</span>':''}</div><div class="stock stock-clean"><span><b>${esc(r.qty)}</b><em>ukupno</em></span><span><b>${esc(r.av)}</b><em>dostupno</em></span><span><b>${esc(r.loan)}</b><em>vani</em></span></div><div class="cm-tools item-actions"><button class="cm-btn" onclick="CleanArmory.editItem('${esc(r.id)}')">Uredi</button><button class="cm-btn bad" onclick="CleanArmory.removeItem('${esc(r.id)}')">Makni</button></div></article>`;
  }
