...
function itemCard(r){const low=r.minimum&&r.av<=r.minimum;return `<article class="item-card ${low?'low-stock':''}"><h3>${esc(r.name)}</h3><div class="muted">${esc(r.category)} · ${esc(r.subcategory)}</div><div class="badgetray"><span class="badge ${low?'bad':(r.av>0?'ok':'bad')}">${low?'ispod praga':esc(r.status)}</span><span class="badge">${esc(r.location||'bez lokacije')}</span>${r.minimum?`<span class="badge warn">prag ${esc(r.minimum)}</span>`:''}${r.type==='rope'?'<span class="badge warn">kodirano uže</span>':''}</div><div class="cm-tools"><button class="cm-btn" onclick="CleanArmory.editItem('${esc(r.id)}')">Uredi artikl</button><button class="cm-btn bad" onclick="CleanArmory.removeItem('${esc(r.id)}')">Makni</button></div></article>`}
