...
function applyFilters(){
  ...
  const selectedVisible = state.selected && state.filtered.some(it=>String(it.object_id)===String(state.selected.object_id));
  if(!selectedVisible){
    state.selected = state.filtered[0] || null;
    if(state.selected){ setTimeout(()=>selectObject(state.selected.object_id),0); }
    else { const d=document.getElementById('detailPanel'); if(d)d.innerHTML='<div class="aw-empty">Odaberi objekt.</div>'; }
  }
  renderList();
}
function renderList(){
