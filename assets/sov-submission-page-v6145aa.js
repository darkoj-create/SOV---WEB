...
function validateDetailed(form,record,files){
  ...
  if(!form.elements.confirm_accuracy.checked)missing.push({name:'confirm_accuracy',label:'potvrda prije predaje'});
  ...
  if(missing.length){
    return {ok:false, first, message:'Nedostaje: '+missing.map(m=>m.label).join(', ')+'.'};
  }
...
function successBanner(id){
  ...
  if(predaja)predaja.scrollIntoView({behavior:'smooth',block:'center'});
}
...
async function boot(){
  ...
  initButtons(); initStepHints(); initFileUX(); initCoordUX(); initScrollSpy();
