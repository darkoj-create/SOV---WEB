-- SOV Oružarstvo v5.40.2 — app catalog search relevance fix
-- Run in Supabase SQL editor with RLS enabled.
-- Fix: removes the global alias soup from search_text. Search tags are now category/subcategory-specific,
-- so typing "croll" no longer matches every rope / every item.
...
as $$
  select case
    ...
    when public.sov_armory_norm(coalesce(raw_category,'') || ' ' || coalesce(search_basis,'')) ~ '(postavlj|karab|hms|matica|spojka|maillon|omni|triact|trilock|screw|twist|sidr|spit|anker|bolt|ploc|ring|fikser)' then 'Oprema za postavljanje'
...
as $$
  select concat_ws(' ',
    case public.sov_armory_main_category(main_category, main_category)
      when 'Užad' then 'uze uzad rope strik staticko staticno dinamicko transportna vreca transportne vrece'
      when 'Oprema za postavljanje' then 'postavljanje karabiner karabin karbiner hms matica spojka maillon omni triact trilock screw lock sidriste sidriste spit anker bolt plocica ring fikser'
      when 'Oprema za crtanje' then 'crtanje crtaci crtaći pribor nacrt mjerenje disto distox topodroid kompas klinometar olovka papir skica'
      when 'Osobna oprema' then 'osobna oprema kaciga helmet pojas sjedalica croll krol crol prsni bloker zumar ascender descender spustalica stop rig maestro pedal stremen'
      when 'Bušilice i baterije' then 'busilica bušilica hilti bosch makita baterija aku punjac punjač svrdlo gbh'
      when 'Elektro i foto oprema' then 'elektro foto kamera video rasvjeta svjetlo lampa ceona čeona'
      when 'Medicinska oprema' then 'medicina medicinska prva pomoc prva pomoć sanitet'
      when 'Oprema za logor' then 'logor kamp bivak sator šator kuhalo plin podloga vreca spavanje'
      when 'Oprema za proširivanje' then 'prosirivanje proširivanje klin cekic čekić macola dlijeto stemanje štemanje'
      when 'Ronilačka oprema' then 'ronjenje ronilacka ronilačka neopren maska peraje boca'
      when 'Alpinistička oprema' then 'alpinisticka alpinistička alpin penjacka penjačka'
      when 'Čisto podzemlje' then 'cisto podzemlje čisto podzemlje ciscenje čišćenje otpad'
      when 'Ostali alat' then 'alat kljuc ključ odvijac odvijač klijesta lopata'
      else null
    end,
    case public.sov_armory_subcategory(subcategory, search_basis)
      ...
      when 'Pojasevi i sjedalice' then 'pojas pojasevi sjedalica sjedni pojas gurtna'
      when 'Descenderi' then 'stop descender spustalica rig id maestro'
      when 'Ručni blokeri' then 'zumar rucni ručni bloker ascender'
      ...
      when 'Baterije' then 'baterija baterije aku'
      when 'Punjači' then 'punjac punjač'
      when 'Svrdla' then 'svrdlo svrdla'
